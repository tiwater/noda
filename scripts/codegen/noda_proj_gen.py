...
def gen_unix_proj(name, tmpl_dir, date_time):
    code_path = name
    tmpl_main = tmpl_dir + 'main_c'
...
def gen_esp32_proj(name, tmpl_dir, date_time):
    code_path = name + '/main'
    tmpl_main = tmpl_dir + 'main_c'
...
def gen_arduino_proj(name, tmpl_dir, date_time):
    code_path = name
    tmpl_main = tmpl_dir + 'main_c'
    gen_proj_entry_file(name, 'ino', tmpl_main, date_time, code_path)

def generate(name, platform):
    if not name:
    ...
    globals()['gen_%s_proj' % platform](name, tmpl_dir, date_time)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='noda_proj_gen')
    parser.add_argument('--name', type=str, help='project name')
    parser.add_argument('--platform', type=str, help='platform: arduino (default) | esp32 | unix')
    args = parser.parse_args()
    generate(args.name, args.platform)
