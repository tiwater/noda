...
int noda_i2c_open(noda_i2c_t* self) {
    ...
    return ESP_OK == i2c_driver_install(self->port, conf.mode, 0, 0, 0)
        ? NODA_OK : NODA_FAIL;
}
...
int noda_i2c_read(noda_i2c_t* self, uint8_t addr, uint8_t reg, uint8_t* data, size_t len, uint16_t wait_ms) {
    return i2c_master_write_read_device(self->port, addr, &reg, 1, data, len, wait_ms / portTICK_PERIOD_MS);
}

int noda_i2c_write_byte(noda_i2c_t* self, uint8_t addr, uint8_t reg, uint8_t data, uint16_t wait_ms) {
    const uint8_t buf[] = { reg, data };
    return i2c_master_write_to_device(self->port, addr, buf, sizeof(buf), wait_ms / portTICK_PERIOD_MS);
} 
