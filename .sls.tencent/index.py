...
def main_handler(event, content):
    ...
    if event["requestContext"]["path"] == "/framework" and event["requestContext"]["httpMethod"] == "POST":
        body = event['body']
        ...
        zip_path = os.path.join('/tmp/ticosframework/', str(folder_r))
        output_path = os.path.join(zip_path, 'output')
       
        os.makedirs(output_path)

        generate(
            'TicosProject',os_set, model, output_path)
        zip_file(output_path, zip_path, 'ticos_framework.zip')
